...
const getStyleKeys = () => {
  console.log(
    bootstrapAvatarColors.map(
      (color) =>
        figma.getLocalPaintStyles().filter((paint) => paint.name == color)[0]
          .key
    )
  );
...
export const nameInit = async () => {
  ...
  for (const selection of h.selection()) {
    if (selection.type == "FRAME" || selection.type == "INSTANCE") {
      const avatars = <InstanceNode[]>(
        selection.findAll(
          (node) =>
            h.isInstance(node) &&
            h.isVariant(node, "Avatar", { Type: "Name" }) &&
            node.visible == true
        )
      );
      console.log(avatars);
      for (const avatar of avatars) {
        ...
        let avatarColorKeys = pixelAvatarColors.map(
          (colorName) => pixelStyles[colorName]
        );
        const isBootstrap =
          h.getComponent(avatar).getSharedPluginData("aperia", "ds") ==
          "bootstrap";
        if (isBootstrap) {
          avatarColors = bootstrapAvatarColors;
          avatarColorKeys = bootstrapAvatarColors.map(
            (colorName) => bootstrapStyles[colorName]
          );
        }
        ...
        while (i < 2 && !found && parentNode.type != "PAGE") {
          const textN = parentNode.findOne(
            (node) =>
              h.isText(node) && node.id != avatarText.id && node.visible == true
          );
        ...
        if (h.isText(avatarText) && textNode) {
          ...
          const hash = isBootstrap
            ? getHashNumberFromString(getAvatarText(textNode.characters))
            : getHashNumberFromString(textNode.characters);
          // const paints = figma.getLocalPaintStyles().filter(paint => paint.name == avatarColors[hash-1]);
          // avatar.fillStyleId = paints[0].id;
          const paint = await figma.importStyleByKeyAsync(
            avatarColorKeys[hash - 1]
          );
