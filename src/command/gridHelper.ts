...
const isGridCol = (node: BaseNode): node is FrameNode => {
  return (
    h.isFrame(node) && node.getSharedPluginData("aperia", "grid_col") == "1"
  );
};
const isGridBody = (node: BaseNode): node is FrameNode => {
  return (
    h.isFrame(node) && node.getSharedPluginData("aperia", "grid_body") == "1"
  );
};
const isGridHead = (node: BaseNode): node is FrameNode => {
  return (
    h.isFrame(node) && node.getSharedPluginData("aperia", "grid_head") == "1"
  );
};
...
const getCellValue = (cell: InstanceNode) => {
  const textNode = <TextNode>(
    cell.findOne((node) => node.type == "TEXT" && node.name == "Value")
  );
...
const getGridInfoFromSelection = (selected: SceneNode) => {
  ...
  if (selected) {
    if (
      selected.type == "FRAME" &&
      selected.findAll((node) => h.isVariant(node, GRID_CELL)).length > 0
    ) {
      grid = selected;
      selectionType = "grid";
    }
    ...
    if (grid) {
      headings = grid
        .findChildren((node) => h.isVariant(node, GRID_HEADING))
        .map((node: InstanceNode) => {
...
const selectSameRowCells = (msg: Message) => {
  ...
  if (h.isVariant(selected, GRID_CELL) || h.isVariant(selected, GRID_HEADING)) {
    ...
    if (selected.parent.type == "FRAME") {
      grid = <FrameNode>selected.parent.parent.parent;
      figma.currentPage.selection = grid.findAll(
        (node) =>
...
const sortGrid = (msg: Message) => {
  ...
  if (girdInfo.grid) {
    const cells = <InstanceNode[]>(
      girdInfo.grid.findChildren((node) => h.isVariant(node, GRID_CELL))
    );
...
export const createGrid = async (msg?: Message) => {
  ...
  let gridCells = h
    .selection()
    .filter((node) => h.isVariant(node, "Grid Cell"));
  console.log(gridCells);
  let gridHeadings = h
    .selection()
    .filter((node) => h.isVariant(node, "Grid Heading"));
  let gridCellsAndHeadings = h
    .selection()
    .filter(
      (node) =>
        h.isVariant(node, "Grid Cell") || h.isVariant(node, "Grid Heading")
    );
  ...
  const widestCell = <InstanceNode>(
    _.orderBy(gridCellsAndHeadings, ["width"], "desc")[0]
  );
  ...
  gridFrame.resizeWithoutConstraints(
    _.sumBy(groupedRows[firstCell.y], "width"),
    gridFrame.height
  );
  ...
  const style =
    h.getLocalPaintStyle("Light/L04") ||
    (await figma.importStyleByKeyAsync(bootstrapStyles["Light/L04"]));
  console.log(style);
...
const updateGridFrame = (gridFrame: BaseNode, resize = true, zebra = true) => {
  ...
  const gridBodyFrame = <FrameNode | null>(
    gridFrame.findChild((node) => isGridBody(node))
  );
  if (!gridBodyFrame) return;

  const colFrames = <FrameNode[]>(
    gridBodyFrame.findChildren((node) => isGridCol(node))
  );
  if (resize) {
    ...
    colFrames.forEach((colFrame, i) => {
      ...
      colFrame.children.forEach((cell: InstanceNode, j) => {
        ...
        if (!(j in highest) || highest[j] < cell.height)
          highest[j] = cell.height;
      });
    });
    // resize
    colFrames.forEach((colFrame, i) => {
...
export const toggleHorizontalScroll = async (msg?: Message) => {
  ...
  for (const gridFrame of h.selection()) {
    if (isGrid(gridFrame)) {
      const Scrollbar = await figma.importComponentSetByKeyAsync(
        bootstrapComponents["Native Scrollbar"].key
      );
      ...
      const checkScrollBar = gridFrame.findChild((node) =>
        h.isVariant(node, Scrollbar.name, { Type: "Horizontal" })
      );
      // if it is already a vertical scrollbar
      const checkVerticalScrollBar = gridBodyFrame
        ? gridBodyFrame.findChild((node) =>
            h.isVariant(node, Scrollbar.name, { Type: "Vertical" })
          )
        : null;
      const HorizontalScrollBar = h.getVariantInSet(Scrollbar, {
        Type: "Horizontal",
        Corner: checkVerticalScrollBar ? "True" : "False",
      });
...
export const toggleVerticalScroll = async (msg?: Message) => {
  ...
  for (const gridFrame of h.selection()) {
    if (isGrid(gridFrame)) {
      ...
      // const Scrollbar = <ComponentSetNode>figma.getNodeById("306:2651");
      const VerticalScrollBar = h.getVariantInSet(Scrollbar, {
        Type: "Vertical",
      });
      if (VerticalScrollBar) {
        ...
        if (!gridBodyFrame) return;
        const checkScrollBar = gridBodyFrame.findChild((node) =>
          h.isVariant(node, Scrollbar.name, { Type: "Vertical" })
        );
        const checkHorizontalScrollBar = <InstanceNode | null>(
          gridFrame.findChild((node) =>
            h.isVariant(node, Scrollbar.name, { Type: "Horizontal" })
          )
        );
        const gridHeadings = <InstanceNode[]>(
          gridFrame.findAll((node) => h.isVariant(node, "Grid Heading"))
        );
        const lastHeadingInner = <FrameNode | null>(
          gridHeadings[gridHeadings.length - 1].findChild(
            (node) => h.isFrame(node) && node.layoutMode == "HORIZONTAL"
          )
        );
        ...
        if (!checkScrollBar) {
          ...
          instance.layoutAlign = "STRETCH";
          lastColFrame.resize(
            lastColFrame.width - instance.width,
            lastColFrame.height
          );
          gridBodyFrame.appendChild(instance);
          if (checkHorizontalScrollBar)
            h.swapVariant(checkHorizontalScrollBar, { Corner: "True" });
          if (lastHeadingInner)
            lastHeadingInner.paddingRight += VerticalScrollBar.width;
        } else {
          lastColFrame.resize(
            lastColFrame.width + checkScrollBar.width,
            lastColFrame.height
          );
          checkScrollBar.remove();
          if (checkHorizontalScrollBar)
            h.swapVariant(checkHorizontalScrollBar, { Corner: "False" });
          if (lastHeadingInner)
            lastHeadingInner.paddingRight -= VerticalScrollBar.width;
        }
      }
...
const getCellValues = (cells: InstanceNode[]) => {
  return cells.map((cell) => {
    const TextLayer = <TextNode | null>(
      cell.findOne((node) => h.isText(node) && node.name == "Text")
    );
...
const handleSortByColumn = (currentColFrame: BaseNode, desc = false) => {
  ...
  const currentColCells = <InstanceNode[]>(
    currentColFrame.findChildren((node) => isGridCell(node))
  );
  ...
  colFrames.forEach((colFrame, i) => {
    ...
    const sortedCells = <InstanceNode[]>colCells.sort((cell1, cell2) => {
      ...
      return desc
        ? cell2Value.localeCompare(cell1Value)
        : cell1Value.localeCompare(cell2Value);
    });
...
export const onSelectionChange = () => {
  ...
  // const gridInfo = getGridInfoFromSelection(selected);
  if (selected) {
    if (selected.getSharedPluginData("aperia", "grid") == "1")
      h.postData({ isGrid: true });
    if (selected.getSharedPluginData("aperia", "grid_col") == "1")
      h.postData({ isGridCol: true });
    if (h.isVariant(selected, GRID_CELL)) h.postData({ isGridCell: true });
    if (h.isVariant(selected, GRID_HEADING))
      h.postData({ isGridHeading: true });
  }
